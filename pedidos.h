...
int enfileiraPedido(FilaPedidos *f, Pedido elem)
{
	if (filaPedidosCheia(f))
		return 0;
	f->tras = (f->tras + 1) % MAX_PEDIDOS; // avanço circular
...
void imprimeFilaPedidos(FilaPedidos *f, Burger *cardapio)
{
	...
	for (int count = 0; count < f->tamanho; count++)
	{
		...
		i = (i + 1) % MAX_PEDIDOS; // Move para o próximo elemento da fila circular
	}
}
...
void geraPedidos(FilaPedidos *f, int num_dia)
{
	...
	int qtdPedidos = base + (rand() % 3); //Quantidade de pedidos a serem gerados ( rand()%3 gerará um número aleatório entre 0 e 2).
