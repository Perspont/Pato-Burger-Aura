...
// É importante diferenciar as estruturas nesta biblioteca. Temos dois dados estruturados: a descritor e o nó da lista
...
//aloca-se memória para o descritor (explicado no início deste arquivo) e faz o seus ponteiros apontarem para o Nulo
...
tp_listad* separa(tp_listad *lista, tp_item e) {
	// Verifica se a lista é válida
	if (lista == NULL || listad_vazia(lista)) {
		return NULL;
	}

	// Encontra o nó que será o último elemento da primeira lista
	tp_no *no_separador = busca_listade(lista, e);

	// Se o elemento não for encontrado ou for o último,
	// não é possível criar uma segunda lista.
	if (no_separador == NULL || no_separador->prox == NULL) {
		return NULL;
	}

	// Aloca memória para o descritor da nova lista
	tp_listad *nova_lista = inicializa_listad();
	if (nova_lista == NULL) {
		// Falha na alocação de memória
		return NULL;
	}

	// Configura a nova lista (lista 2)
	nova_lista->ini = no_separador->prox;
	nova_lista->fim = lista->fim;

	// Modifica a lista original (lista 1) para terminar no nó separador
	lista->fim = no_separador;

	// Quebra as conexões entre as duas listas
	nova_lista->ini->ant = NULL; // O novo início da lista 2 não tem anterior
	lista->fim->prox = NULL;     // O novo fim da lista 1 não tem próximo
